...
use crate::osm_pbf_file::pbf_fixtures::{self, blob_compression, header_blob, header_spec};
...
fn setup(tag: &str, spec: &header_spec, compression: blob_compression) -> scene {
  ...
  let file_id = crate::osm_pbf_file::repository::ensure_by_file_path(&conn, &temp.pbf_path);
