...
use crate::osm_node::osm_node_row;
use crate::osm_relation::osm_relation_row;
use crate::osm_way::osm_way_row;
use crate::osm_node::decoder::{
  block_scale, decode as decode_nodes, decode_dense as decode_dense_nodes,
};
use crate::osm_node::osm_node;
use crate::osm_relation::decoder::decode as decode_relations;
use crate::osm_relation::osm_relation;
use crate::osm_tag::tag_policy;
use crate::osm_way::decoder::decode as decode_ways;
...
fn writer_thread(
  ...
  std::thread::spawn(move || {
    while let Some(mut state) = wait_for_flush(&write_buf) {
      ...
      crate::osm_node::repository::insert_rows(&tx, &batch.nodes);
      crate::osm_way::repository::insert_rows(&tx, &batch.ways);
      crate::osm_relation::repository::insert_rows(&tx, &batch.relations);
