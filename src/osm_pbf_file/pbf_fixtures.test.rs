...
use crate::osm_pbf_file::message::{
...
pub(crate) fn insert_node(
  ...
  let node = crate::osm_node::osm_node {
    id: id as i64,
    lat,
    lon,
    tags: tag_map(tags),
  };
  let row = crate::osm_node::osm_node_row::encode(
    &node,
    0,
    &mut crate::database::jsonb::encoder::new(),
  );
  crate::osm_node::repository::insert_rows(conn, &[row]);
}
...
pub(crate) fn insert_way(
  ...
  let way = crate::osm_way::osm_way {
    id: id as i64,
    refs: refs.to_vec(),
    tags: tag_map(tags),
  };
  let row = crate::osm_way::osm_way_row::encode(
    &way,
    0,
    &mut crate::database::jsonb::encoder::new(),
  );
  crate::osm_way::repository::insert_rows(conn, &[row]);
}
...
pub(crate) fn insert_relation(
  ...
  let row = crate::osm_relation::osm_relation_row::encode(
    &relation,
    0,
    &mut crate::database::jsonb::encoder::new(),
  );
  crate::osm_relation::repository::insert_rows(conn, &[row]);
}
...
pub(crate) fn indexed_scene(tag: &str, chunks: &[Vec<u8>]) -> (temp_scene, u32) {
  ...
  let file_id = crate::osm_pbf_file::repository::ensure_by_file_path(&conn, &scene.pbf_path);
  crate::osm_pbf_file::blob_scanner::run(&scene.pbf_path, &conn, file_id, |_| {});
  drop(conn);
