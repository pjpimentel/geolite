...
use crate::osm_pbf_file::pbf_fixtures::{
...
fn run_scene(tag: &str, chunks: &[Vec<u8>], opts: data_opts, threads: u8) -> outcome {
  ...
  let blob_chunks = crate::osm_pbf_file::blob_index::get_data_chunks(&conn, file_id);
  drop(conn);
...
fn _00_13_second_run_does_not_duplicate_rows() {
  ...
  let file_id = crate::osm_pbf_file::repository::ensure_by_file_path(&conn, &out.scene.pbf_path);
  let blob_chunks = crate::osm_pbf_file::blob_index::get_data_chunks(&conn, file_id);
  drop(conn);
...
fn _01_03_writer_notifies_decoders_after_draining_a_full_buffer() {
  ...
  let buffer = test_write_buffer(1, 1);
  {
    ...
    state.current.nodes.push_back(crate::osm_node::osm_node_row {
      id: 1,
      osm_pbf_chunk_id: 1,
      payload: vec![0x0c],
    });
...
fn _02_00_decoded_blob_bytes_sums_stack_and_heap() {
  ...
  let filled = decoded_blob {
    nodes: vec![crate::osm_node::osm_node_row {
      id: 1,
      osm_pbf_chunk_id: 1,
      payload: vec![0u8; 64],
    }],
...
fn _02_01_buffer_row_count_sums_every_queue() {
  ...
  data.nodes.push_back(crate::osm_node::osm_node_row {
    id: 1,
    osm_pbf_chunk_id: 1,
    payload: Vec::new(),
  });
  data.ways.push_back(crate::osm_way::osm_way_row {
    id: 2,
    osm_pbf_chunk_id: 1,
    payload: Vec::new(),
  });
  data
    .relations
    .push_back(crate::osm_relation::osm_relation_row {
      id: 3,
      osm_pbf_chunk_id: 1,
      payload: Vec::new(),
    });
...
fn _02_02_decode_blob_uses_an_empty_string_table_when_absent() {
  ...
  let block = crate::osm_pbf_file::message::primitive_block_msg {
    stringtable: None,
    primitivegroup: vec![crate::osm_pbf_file::message::primitive_group_msg {
      nodes: Vec::new(),
      dense: Some(crate::osm_pbf_file::message::dense_nodes_msg {
        id: vec![1],
        denseinfo: None,
        lat: vec![387_000_000],
        lon: vec![-91_000_000],
...
fn _02_03_decode_blob_tolerates_invalid_utf8_in_string_table() {
  ...
  let block = crate::osm_pbf_file::message::primitive_block_msg {
    stringtable: Some(crate::osm_pbf_file::message::string_table_msg {
