...
pub struct encoder {
  // pool of reusable scratch buffers, popped on alloc and pushed back on free: the payloads
  // written today nest at most three levels (an object holding an array of objects), so three
  // scratches are active at once and no row allocates after warmup
...
impl encoder {
  ...
  pub(crate) fn write_object<F>(&mut self, out: &mut Vec<u8>, f: F)
  where
  ...
  pub(crate) fn write_array<F>(&mut self, out: &mut Vec<u8>, f: F)
  where
    F: FnOnce(&mut encoder, &mut Vec<u8>),
  {
    let mut scratch = self.alloc();
