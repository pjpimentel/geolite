...
pub(super) fn run(
  ...
  let candidate_ids =
    crate::osm_way::repository::remaining_ids_by_tags(conn, level::street, exclude);
...
fn load_chunk(conn: &Connection, chunk: &[u64], name_priority: &[&str]) -> Vec<way_work> {
  let raw_rows = crate::osm_way::repository::way_coords_chunk(conn, chunk, name_priority);
